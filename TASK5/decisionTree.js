...
var height = 50;                                                         //height of attribute label in tree
var width = 150;                                                          //width of attribute label in tree
...
function init() {
    ...
    canvas.height = height * (type.length - 1 + ((type.length - 2) * 2));
    document.documentElement.scrollHeight = canvas.height + 50 * window.innerHeight;
    ...
    canvas.width = width * (nb_subattribute + ((nb_subattribute + 1) * 0.5));
    document.documentElement.scrollWidth = canvas.width + 50 * window.innerWidth;
}
    ...
    for (let i = 1; i < tmp[u].length; i++) {
        if (tmp[u][i] == s) {
            ...
            if (tmp[tmp.length - 1][i] == "yes") {
                console.log(tmp[tmp.length - 1][i], "was equal to yes");
                pos++
            }
            else {
                neg++;
